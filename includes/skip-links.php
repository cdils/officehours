...
add_action ( 'genesis_before_header', 'podcast_pro_skip_links', 5 );
/**
 * Add skiplinks for screen readers and keyboard navigation
 *
 * @since  1.0.0
 */
function podcast_pro_skip_links() {

	// Call function to add IDs to the markup
	utility_skiplinks_markup();

	// write HTML, skiplinks in a list with a heading
	echo '<h2 class="screen-reader-text">'. __( 'Skip links', 'podcast-pro' ) .'</h2>' . "\n";

	echo '<ul class="wpacc-genesis-skip-link">' . "\n";

	if ( has_nav_menu( 'primary' ) ) {
		echo '  <li><a href="#genwpacc-genesis-nav-primary" class="screen-reader-shortcut">'. __( 'Skip to primary navigation', 'podcast-pro' ) .'</a></li>' . "\n";
	}

	echo '  <li><a href="#genwpacc-genesis-content" class="screen-reader-shortcut">'. __( 'Skip to content', 'podcast-pro' ) .'</a></li>' . "\n";

	if ( ( 'sidebar-content' === genesis_site_layout() ) || ( 'content-sidebar' === genesis_site_layout() ) ) {
		echo '  <li><a href="#genwpacc-sidebar-primary" class="screen-reader-shortcut">'. __( 'Skip to primary sidebar', 'podcast-pro' ) .'</a></li>' . "\n";
	}
	...
	if ( isset( $footer_widgets[0] ) && is_numeric( $footer_widgets[0] ) ) {
		echo '  <li><a href="#genwpacc-genesis-footer-widgets" class="screen-reader-shortcut">'. __( 'Skip to footer widgets', 'podcast-pro' ) .'</a></li>' . "\n";
	}

	if ( has_nav_menu( 'footer' ) ) {
		echo '  <li><a href="#genwpacc-genesis-nav-footer" class="screen-reader-shortcut">'. __( 'Skip to footer navigation', 'podcast-pro' ) .'</a></li>' . "\n";
	}
...
function utility_skiplinks_markup() {
	...
	add_filter( 'genesis_attr_nav-primary', 'podcast_pro_genesis_attr_nav_primary' );
	add_filter( 'genesis_attr_content', 'podcast_pro_genesis_attr_content' );
	add_filter( 'genesis_attr_sidebar-primary', 'podcast_pro_genesis_attr_sidebar_primary' );
	add_filter( 'genesis_attr_footer-widgets', 'genesis_attr_footer_widgets' );
	add_filter( 'genesis_attr_nav-footer', 'podcast_pro_genesis_attr_nav_footer' );
}

// Add ID markup if primary nav is assigned to a menu area
// We use have_nav_menu()
function podcast_pro_genesis_attr_nav_primary( $attributes ) {
	$attributes['id'] = 'genwpacc-genesis-nav-primary';
	return $attributes;
}

// Add ID markup to content area
function podcast_pro_genesis_attr_content( $attributes ) {
	$attributes['id'] = 'genwpacc-genesis-content';
	return $attributes;
}

// Add ID markup if the primary sidebar is active
function podcast_pro_genesis_attr_sidebar_primary( $attributes ) {
...
function podcast_pro_genesis_attr_nav_footer( $attributes ) {
